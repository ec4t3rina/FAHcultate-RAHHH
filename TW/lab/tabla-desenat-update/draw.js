   ...
   const colorInput = document.getElementById("colorPicker");
    ...
    container.addEventListener("click", function(e) {
        if (e.target.tagName === "TD") {
            ...
            // găsește clasa r0, r1, ... și c0, c1,...
            let rowClass = Array.from(cell.classList).find(c => c.startsWith("r"));
            let colClass = Array.from(cell.classList).find(c => c.startsWith("c"));

            if (!rowClass || !colClass) return;

            let row = parseInt(rowClass.substring(1));
            let col = parseInt(colClass.substring(1));

            drawPixel(row, col, colorInput.value);
        }
    });
